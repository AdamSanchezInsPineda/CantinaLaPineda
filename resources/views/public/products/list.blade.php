                ...
                <a href="{{ route('product.show', $product->id) }}" data-product-name="{{ $product->name }}">
                    <div class="flex flex-col w-[320px] h-[400px]">
                        <div class="w-[310px] h-[320px] flex items-center justify-center">
                            <img src="{{ $product->images && !empty($product->images) ? asset('storage/' . $product->images[0]) : "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTy7S0JruZGX6NJHRNy3XP60n62PnJWIR-4Iw&s"}}" alt="Imagen de {{ $product->name }}" class="max-w-[300px] max-h-[310px] min-w-[300px] min-h-[310px] rounded-md">
                        </div>
